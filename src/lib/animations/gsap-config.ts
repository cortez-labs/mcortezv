...
export function initGSAP() {
	...
	document.querySelectorAll('a[href^="#"]').forEach(anchor => {
		anchor.addEventListener('click', function (e: Event) {
			e.preventDefault();
			const target = document.querySelector((e.currentTarget as HTMLElement).getAttribute('href') || '');
			if (target) {
				gsap.to(window, {
					duration: 1.5,
					scrollTo: target,
...
/**
 * Advanced text reveal animation using split-type
 */
export function animateTextReveal(element: HTMLElement, options = {}) {
	...
	// Import split-type dynamically
	import('split-type').then(({ default: SplitType }) => {
		const splitText = new SplitType(element, { types: config.type as string });
		
		// Set initial state
		gsap.set(splitText[config.type as string], config.from);
		
		// Animate
		const tl = gsap.timeline();
		tl.to(splitText[config.type as string], {
			...config.to,
 ...
 */
export function animateStagger(elements: NodeListOf<Element> | Element[], options = {}) {
	const config = {
		from: { y: 50, opacity: 0 },
		to: { y: 0, opacity: 1 },
		duration: 0.8,
 ...
 */
export function createParallax(element: HTMLElement, options = {}) {
	const config = {
		speed: 0.5,
		direction: 'vertical',
		ease: 'none',
 ...
 */
export function create3DCardFlip(card: HTMLElement) {
	if (!card) return;
 ...
 */
export function createShimmerEffect(element: HTMLElement) {
	if (!element) return;
 ...
 */
export function createPageTransition(container: HTMLElement) {
	if (!container) return;
